...
**Q3.** Tidy the genomic data by finishing the relevant parts of code chunk below (participants should replace "***" with their own code).
```
...
combined_long <- dplyr::full_join(***, by  = "Key") %>% 
...
combined_long <- combined_long %>% 
  ...
  dplyr::mutate(
    ...
    gt = dplyr::case_when(gt_GT == "0/0" ~ 0,
                          gt_GT == ***, # Het calls should be replaced with 0.5 
                          gt_GT == "1/1" ~ 1), 
    ...
    wsraf = ***, # referent allele depth (rad) should be in numerator
...
**Conceptual Q1:** As COI increase, what is the expected relationship of proportion of loci that are heterozygous?
`r begin_button(101)`
**Answer:** As COI increases, more loci will have heterozygous genotype calls.
`r end_button()`
`r hrule()`
<br>
  
**Conceptual Q2:** What advantage is there for sequencing multiple loci if you suspect there is a high COI?
`r begin_button(102)`
**Answer:** More loci will decrease the probability that our data contains false-negative genotype calls, where for example if we had three clones with coin toss results of: HHH. As a result, more loci have more opportunities to find heterozygous genotype calls.  
`r end_button()`
`r hrule()`
<br>
...
**Conceptual Q3:** Using the "Coin Toss" conceptual framework, describe how within sample allele frequencies may change as COI increases (where H = 0, T = 1)?

`r begin_button(103)`
**Answer:** As COI increases, expect WSAF to have more mass in intermediate alele frequency ranges and less mass at  0% and 100% allele frequency.
`r end_button()`
`r hrule()`
<br>
