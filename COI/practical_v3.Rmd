...
## Running RMCL
Now run the RMCL using the default settings provided in the [Github](https://github.com/EPPIcenter/THEREALMcCOIL/) for the DRC and VN. Note, you will need to create an output directory for results to send your output file to. 
...
**Q10:** Using `tidyverse` commands, compute the fraction of mixed infectios for each country.
...
```{r}
combined_COI %>% group_by(country) %>% summarise(frac_mixed=sum(mean>1)/n())
```
...
**Q11:** Do the Fws conform to expectation? Explain.
