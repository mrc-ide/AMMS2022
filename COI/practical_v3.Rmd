...
**Q&**: Complete the code chunk below (filling in *** with your own code) to convert the data into a RMCL format for VN using the DRC as an example.  
...
**Q&**: Complete the code chunk below (filling in *** with your own code) to filter your THEREALMcCOIL results to the relevant bits for VN using the DRC as an example. Then combine your results into one large dataframe.
...
**Q&**: Complete the code chunk below (filling in *** with your own code) to make a boxplot of your results.  
...
**Q&:** Are there any differences in COI between the two countries? If so, can you explain them?

`r begin_button(104)`

**A&:** _Yes, the COI estimates are generally higher in DRC than in VN. This could be explained by differences in transmission intensity between the two countries._

`r end_button()`

**Q&:** Using `tidyverse` commands: `group_by` and `summarise`, compute the fraction of mixed infectios for each country.
...
**Q&:** Complete the code chunk below to calculate the WSAF per sample.
...
**Q&:** Do the Fws conform to expectation? Explain.

`r begin_button(108)`

**A11:** _Here, we see that DRC has *lower* Fws than VN. Again, this would be expected if transmission intensity were higher in the DRC._
...
**Q&:** Using the code above, now make a boxplot for number of heterozygous sites versus complexity of infection. The resulting figure should resemble the plot below: 
