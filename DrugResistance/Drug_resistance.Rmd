...
As we discussed in the lecture, for some drugs the combination of alleles (haplotypes) is the most relevant information. Next, we will use a packge, UpSetR, to visualize combinations of mutations. 

Documentation on the package can be found here:
https://cran.r-project.org/web/packages/UpSetR/UpSetR.pdf


Polyclonal data presents a challenge for haplotypes: if a sample contains 2 alleles for 2 loci, which allele goes with which in each of the parasites in that infection? 
...
dr.data.simplified %>%
  ...
  mutate(haplotype = paste(`437`,`540`,`581`)) %>% 
  group_by(haplotype) %>%  
  summarize( n = n())
```
