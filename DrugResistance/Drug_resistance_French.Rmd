---
title: "Activité AMMS Pratique : Résistance aux médicaments"
...
Les données génétiques peuvent être agrégées ou par échantillon. Les informations contenues dans un ensemble de données simple par échantillon doivent inclure un identifiant d'échantillon, des informations sur le locus (position ou un autre identifiant, séquence de référence et alternative) et l'abondance de séquences de référence et alternatives dans cette région du génome. Les métadonnées peuvent également être incluses. Pour les échantillons, nous souhaiterons peut-être savoir quand et où ils ont été collectés, ainsi que toute autre information pertinente. Pour ces régions du génome, nous pouvons vouloir connaître, par exemple, les séquences d'acides aminés pour lesquelles codent les codons.

Dans ce TP, nous utiliserons un ensemble de données qui viennent de la République Démocratique du Congo en 2020. Nous avons simplifié l'ensemble de données originales pour plus faciliment completer l'exercise. Nous examinerons les mutations liées à la résistance dans le gène dhps.
...
La dernière colonne est appelée REF_WSAF et décrit la proportion de séquences de référence dans une région du génome particulière et un échantillon donné.

Jetons maintenant un coup d'œil à dr.data. Parce que dr.data a 1617 lignes, on ne peut pas l'afficher en entier alors, n'affichons que les 10 premières lignes.

```{r}
dr.data[1:10,]
```

Comme vous pouvez le constater, chaque échantillon comporte plusieurs lignes. Cela signifie probablement que nos données sont dans un format long, où les informations pour un échantillon sont contenues dans différentes lignes, au lieu de plusieurs colonnes.
En outre, certaines lignes affichent des valeurs NA dans REF_WSAF. Cela signifie que dans cet échantillon, il n'y a aucune information pour cette donnée.

**Q2.** Pourquoi voudrions-nous stocker les données dans un format long ?

`r begin_button(2)`

**A2.** Le format long est utile pour enregistrer des valeurs irrégulières ou manquantes, et est généralement considéré  plus efficace. Un avantage majeur pour nos besoins est qu'il nous permet d'utiliser les fonctions `ggplot2`, qui attendent des données au format long en entrée.
...
**Q6.** Est-ce que votre réponse à la question "Quel codon a une prévalence plus élevée ?" change maintenant que vous avez vu des intervalles de confiance ?
...
Comme nous en avons discuté dans la lecture, pour certains médicaments, la combinaison d'allèles (haplotypes) est l'information la plus pertinente. Ensuite, nous utiliserons un package, `UpSetR`, pour visualiser les combinaisons de mutations. La documentation sur le package peut être trouvée [ici](https://cran.r-project.org/web/packages/UpSetR/UpSetR.pdf ).

Les données polyclonales présentent un défi pour la construction d'haplotypes. Par exemple, si un échantillon contient 2 allèles à chacun des 2 régions du génome, comment pouvons-nous savoir quel allèle va avec lequel dans chacun des parasites de cette infection ? Ce problème est appelé **phasage**, et nous travaillons souvent avec des données **non phasées**.
...
Nous pouvons voir que certaines combinaisons d'haplotypes sont plus communes que d'autres. Par exemple, le triple mutant est très commun, tout comme le double mutant 540+581.
